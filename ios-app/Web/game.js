...
// App ID:     ca-app-pub-9218266514966883~9453060593
// Ad Unit ID: ca-app-pub-9218266514966883/8264654318

// ─── TEMA ────────────────────────────────────────────────────────────────────
const DEV_MODE = false; // App Store sürümünde geliştirici kontrolleri kapalıdır.
const THEME = {
...
function drawLevelGridCell(i, offsetX, offsetY, now, gridOpacity) {
  ...
  const levelBarH = 26;
  const BANNER_H = window.COLOR_SPLIT_STORE_SCREENSHOT ? 0 : 60;
  const barH = Math.min(80, H * 0.1);
  const bottomBarY = H - 18 - barH - BANNER_H;
  ...
  if (gridOpacity > 0.001) {
    ...
    if (i < transition.fromIdx) {
      ...
      ctx.fillText('✓ TAMAMLANDI', W / 2, H - 45 - BANNER_H);
    } else if (i === transition.fromIdx) {
      ctx.fillStyle = THEME.win;
      ctx.fillText('✓ SEVİYE TAMAM', W / 2, H - 45 - BANNER_H);
    } else {
      ctx.fillStyle = THEME.accent;
      ctx.fillText('KİLİTLİ', W / 2, H - 45 - BANNER_H);
    }
    ctx.restore();
  }
...
function doCoinBonus() {
  ...
  const nativeRewardedAd = window.webkit &&
    window.webkit.messageHandlers &&
    window.webkit.messageHandlers.colorSplitRewardedAd;

  if (nativeRewardedAd) {
    if (rewardedAdPending) return;
    rewardedAdPending = true;
    nativeRewardedAd.postMessage({ reward: 'coinBonus' });
    showToast('Reklam hazırlanıyor…', THEME.dim, 1500);
    return;
  }
...
// WKWebView, ödüllü reklam sonucunu bu fonksiyonla oyuna iletir.
window.colorSplitRewardedAdResult = function (success, reason) {
  if (!rewardedAdPending) return;
  ...
  const message = reason === 'dismissed'
    ? 'Ödül için reklamı tamamla'
    : 'Reklam şu anda hazır değil';
  showToast(message, THEME.dim, 1800);
};
...
function layout() {
  ...
  const bottomBar = { x: W / 2 - barW / 2, y: H - 18 - barH - BANNER_H, w: barW, h: barH };
  const banner = { x: 0, y: H - BANNER_H, w: W, h: BANNER_H };
  ...
  const btns = [];
  const ids = ['hint', 'undo'];
  const costs = [HINT_COST, UNDO_COST];
  const bw = bottomBar.w / 2;
  for (let k = 0; k < 2; k++) {
    btns.push({ id: ids[k], cost: costs[k], x: bottomBar.x + k * bw, y: bottomBar.y, w: bw, h: bottomBar.h });
  }
  return { coinPill, restart, levelY, levelBarH, rects, bottomBar, btns, banner };
}
...
function drawLiquidBand(x, w, top, height, ci, sprinkleKey, angle, skipTopCap, flatBottom) {
  ...
  // If flatBottom is true, the bottom cap is horizontal (rotation 0)
  const useBottomTilt = !flatBottom && clampedRotAngle !== 0;
  ...
  if (useBottomTilt) {
    ctx.lineTo(x + w, botY - dy);
    ctx.ellipse(x + rx_base, botY, rx, ry, clampedRotAngle, 0, Math.PI, false);
  } else {
    ctx.lineTo(x + w, botY);
...
function drawTube(R, spec) {
  ...
  const dir = angle < 0 ? -1 : (angle > 0 ? 1 : 0);
  ...
  // Durgun (sabit) sıvı katmanları - asla esnemez ve daralmaz, şişe dibinde sabit kalır
  for (let s = 0; s < spec.units.length; s++) {
    const segBottom = ibot - s * segH;
    const sameAbove = (s + 1 < spec.units.length && spec.units[s] === spec.units[s + 1]) ||
      (s + 1 === spec.units.length && spec.partialTop && spec.units[s] === spec.partialTop.color);
    drawLiquidBand(ix, iw, segBottom - segH - 0.5, segH + 1, spec.units[s], R.i * 17 + s, 0, sameAbove);
  }

  let currentBase = spec.units.length;
  if (spec.partialTops && spec.partialTops.length) {
    // Alıcı (incoming) şişe - sadece dikey dolum (dik durduğu için esneme olmaz)
    for (let k = 0; k < spec.partialTops.length; k++) {
      ...
      if (pt.frac > 0.001) {
        ...
        const sameAbove = (k + 1 < spec.partialTops.length && pt.color === spec.partialTops[k + 1].color);
        drawLiquidBand(ix, iw, segBottom - ph, ph + 0.5, pt.color, R.i * 17 + currentBase + 9 + k, 0, sameAbove);
        currentBase += pt.frac;
      }
    }
  } else if (spec.partialTop && spec.partialTop.frac > 0.001) {
    // Dökülen (outgoing) aktif sıvı katmanı
    const segBottom = ibot - currentBase * segH;
    const frac = spec.partialTop.frac;

    // Tüm dökümlerde (hacim gözetmeksizin) yer çekimi esneme ve daralma animasyonu kullanılır.
    const tiltPct = Math.min(1, Math.abs(angle) / POUR_ANGLE);

    let stretchFactor = 1.0;
    if (dir !== 0 && frac > 0.001) {
      const availableSlots = CAP - currentBase;
      stretchFactor = 1 + ((availableSlots - frac) / frac) * tiltPct;
    }

    const segH_stretched = segH * stretchFactor;
    const drawW = iw / stretchFactor;
    const drawX = (dir > 0) ? ix + iw * (1 - 1 / stretchFactor) : ix;
    const drawH = segH_stretched * frac;

    // Dökülen segmentin alt sınırı düz durur (flatBottom = true), üst yüzeyi ise yer çekimine paralel yatar (angle = angle).
    drawLiquidBand(drawX, drawW, segBottom - drawH, drawH + 0.5, spec.partialTop.color, R.i * 17 + currentBase + 9, angle, false, true);
  }
  ctx.restore();
...
function drawBannerPlaceholder(L) {
  if (window.COLOR_SPLIT_STORE_SCREENSHOT) return;
  const B = L.banner;
  if (!B) return;

  ctx.save();
  // 1. Reklam alanı arka planı (koyu lacivert panel)
  ctx.fillStyle = '#0a0d1d';
  ctx.fillRect(B.x, B.y, B.w, B.h);

  // 2. Üst ince ayırıcı çizgi
  ...
  // 3. Kesikli çerçeveli reklam kartı
  const paddingX = 16;
  const paddingY = 6;
  const adW = B.w - 2 * paddingX;
  const adH = B.h - 2 * paddingY;

  roundRectPath(B.x + paddingX, B.y + paddingY, adW, adH, 10);
  ...
  // 4. Reklam alanı ve AdMob Birim ID metinleri
  ctx.fillStyle = 'rgba(255, 255, 255, 0.22)';
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.font = `600 ${Math.floor(adH * 0.28)}px 'Segoe UI', sans-serif`;
  ctx.fillText('REKLAM ALANI (BANNER)', B.x + B.w / 2, B.y + B.h * 0.44);

  ctx.fillStyle = 'rgba(255, 255, 255, 0.12)';
  ctx.font = `500 ${Math.floor(adH * 0.18)}px 'Segoe UI', sans-serif`;
  ctx.fillText('ca-app-pub-9218266514966883/8264654318', B.x + B.w / 2, B.y + B.h * 0.76);
